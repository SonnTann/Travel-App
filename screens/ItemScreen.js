        ...
        <SafeAreaView className="flex-1 bg-white relative">
          <ScrollView className="flex-1 px-4 py-6">
            <View className="relative bg-white shadow-lg">
              ...
              <View className=" absolute inset-x-0  justify-between flex-row px-6">
                <TouchableOpacity
                ...
                <TouchableOpacity className="bg-[#06B2BE] rounded-md w-10 h-10 items-center justify-center">
                  <FontAwesome name="heartbeat" size={24} color="#fff" />
                </TouchableOpacity>
              </View>
              <View className=" absolute inset-x-0  bottom-5 justify-between flex-row px-6">
           ...
           {data?.description &&(
            <Text className="mt-4 text-[16px] font-semibold text-[#97A6AF]">{data?.description}</Text>
           )}
    
           {data?.cuisine &&(
            <View className="flex-row gap-2 items-center justify-start flex-wrap mt-4">{data?.cuisine.map((n)=>(
              <TouchableOpacity
              key={n.key}
